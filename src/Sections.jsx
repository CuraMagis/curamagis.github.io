...
function Process() {
  ...
  return (
    <SectionShell bg="white" id="process">
      ...
      <Reveal delay={140}>
        <p style={{ fontFamily: "var(--font-display)", fontSize: 20, lineHeight: 1.5, color: "var(--navy)", margin: "30px 0 0", paddingTop: 22, borderTop: "1px solid var(--gold)", maxWidth: "64ch" }}>
          A family who begins in ninth grade moves through all four chapters with us, not one. A family who joins later steps in where they are, and we carry forward what the earlier years would have set in place. The earlier we start, the more we can do.
        </p>
      </Reveal>
    </SectionShell>);
...
function Testimonials() {
  ...
  return (
    <SectionShell bg="paper" id="families" style={{ position: "relative", overflow: "hidden" }}>
      <div aria-hidden="true" className="cm-wallpaper"
        style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", justifyContent: "space-between", opacity: 0.5, pointerEvents: "none", userSelect: "none", zIndex: 0 }}>
        {rows.map((row, r) =>
        <div key={r} style={{ overflow: "hidden", display: "flex" }}>
            <div className="cm-wallpaper-row" style={{ display: "flex", flex: "0 0 auto", whiteSpace: "nowrap", fontFamily: "var(--font-body)", fontSize: "var(--fs-caption)", letterSpacing: "var(--tracking-eyebrow)", color: "var(--accent-ink)", animation: `${row.dir} ${row.dur}s linear infinite` }}>
      ...
      <Reveal style={{ maxWidth: 680, marginBottom: 40, position: "relative", zIndex: 1 }}>
        <span className="cm-scrim"><Eyebrow>From Families</Eyebrow></span>
      </Reveal>
      <Reveal delay={80} style={{ maxWidth: 900, margin: "0 auto", position: "relative", zIndex: 1 }}>
        <div onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}
          style={{ display: "grid" }}>
          {quotes.map(([q, who, where], i) =>
          <figure key={who + where} id={`cm-quote-${i}`} aria-hidden={i !== active}
            style={{ gridArea: "1 / 1", margin: 0, display: "flex", flexDirection: "column", justifyContent: "center", background: "var(--white)", border: "1px solid var(--line)", borderTop: "3px solid var(--gold)", borderRadius: "var(--radius-md)", boxShadow: "var(--shadow-sm)", padding: "44px 52px 40px", opacity: i === active ? 1 : 0, transform: i === active ? "translateY(0)" : "translateY(8px)", transition: "opacity 420ms var(--ease-out), transform 420ms var(--ease-out)", pointerEvents: i === active ? "auto" : "none" }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", marginTop: 20 }}>
          <span className="cm-scrim" style={{ display: "flex", gap: 4, padding: "0 8px" }}>
        ...
        <p style={{ margin: "4px 0 0", textAlign: "right" }}>
          <span className="cm-scrim" style={{ display: "inline-block", fontFamily: "var(--font-body)", fontSize: "var(--fs-caption)", color: "var(--ink-muted, var(--ink-soft))" }}>
...
function Engagement({ onContact }) {
  ...
  return (
    <SectionShell bg="paper2" id="engagements">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "0.85fr 1.15fr", gap: 64, marginTop: 40, alignItems: "start" }} className="cm-grid-2-lg">
        <Reveal delay={80} style={{ order: 2 }}>
          <dl style={{ margin: 0, display: "flex", flexDirection: "column", gap: 26 }}>
            {consult.map(([t, d], i) =>
            <div key={t} style={{ display: "grid", gridTemplateColumns: "38px 1fr", gap: 18, alignItems: "start" }}>
                <span style={{ width: 38, height: 38, borderRadius: "50%", border: "1px solid var(--gold)", color: "var(--accent-ink)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "var(--font-display)", fontSize: 18, fontWeight: 600 }}>{i + 1}</span>
                <div>
                  <dt style={{ fontFamily: "var(--font-display)", fontSize: 23, fontWeight: 600, color: "var(--navy)", marginBottom: 6, lineHeight: 1.25 }}>{t}</dt>
                  <dd style={{ margin: 0, fontFamily: "var(--font-body)", fontSize: 16.5, lineHeight: 1.6, color: "var(--ink-soft)", maxWidth: "56ch" }}>{d}</dd>
                </div>
              </div>
            )}
          </dl>
          <p style={{ fontFamily: "var(--font-body)", fontSize: 16, lineHeight: 1.6, color: "var(--ink-soft)", margin: "30px 0 0", paddingTop: 22, borderTop: "1px solid var(--line)", fontStyle: "italic", maxWidth: "58ch" }}>
        ...
        <Reveal delay={160} style={{ order: 1 }}>
          <div style={{ background: "var(--white)", border: "1px solid var(--line)", borderTop: "3px solid var(--gold)", borderRadius: "var(--radius-lg)", padding: "34px 32px", boxShadow: "var(--shadow-sm)" }}>
            <h3 style={{ fontFamily: "var(--font-display)", fontSize: 26, fontWeight: 600, color: "var(--navy)", margin: "0 0 22px", lineHeight: 1.2 }}>How our engagements work</h3>
            <dl style={{ margin: 0, display: "flex", flexDirection: "column", gap: 20 }}>
              {terms.map(([t, d]) =>
              <div key={t}>
                  <dt style={{ display: "flex", gap: 10, alignItems: "flex-start", fontFamily: "var(--font-body)", fontSize: 16.5, fontWeight: 600, color: "var(--navy)", marginBottom: 5 }}>
                    <span style={{ color: "var(--gold)", flex: "none", marginTop: 1 }}><Icon name="check" size={18} /></span>
                    <span>{t}</span>
                  </dt>
                  <dd style={{ margin: 0, paddingLeft: 28, fontFamily: "var(--font-body)", fontSize: 15.5, lineHeight: 1.55, color: "var(--ink-soft)" }}>{d}</dd>
                </div>
              )}
            </dl>
          </div>
