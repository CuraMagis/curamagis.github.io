  ...
  return (
    <section id="katie" style={{ position: "relative", background: "var(--grad-navy)", overflow: "hidden", paddingTop: 116, paddingBottom: 104 }}>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 32px", position: "relative" }}>
        <div className="cm-hero-logo" style={{ opacity: scrolled ? 0 : 1, transform: scrolled ? "translateY(-10px)" : "none", transition: "opacity var(--dur) var(--ease-out), transform var(--dur) var(--ease-out)", marginBottom: 26 }}>
          <Logo variant="white" height="min(52px, 12vw)" />
          <p style={{ fontFamily: "var(--font-body)", fontSize: 13, fontWeight: 600, letterSpacing: "0.18em", textTransform: "uppercase", color: "var(--gold-mid)", lineHeight: 1.5, margin: "16px 0 0" }}>
            College Admissions Counseling
          </p>
        </div>
        <div className="cm-hero-rail" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "clamp(16px, 2.4vw, 34px)", paddingTop: 26, borderTop: "1px solid rgba(255,255,255,0.16)" }}>
          {rail.map(([a, b]) => (
            <div key={a} style={{ borderLeft: "2px solid var(--gold)", paddingLeft: "clamp(12px, 1.2vw, 18px)" }}>
              <div style={{ fontFamily: "var(--font-body)", fontSize: "clamp(15px, 1.35vw, 17px)", fontWeight: 700, color: "var(--white)", lineHeight: 1.4 }}>{a}<br />{b}</div>
            </div>
          ))}
        </div>
        <div className="cm-hero-grid" style={{ display: "grid", gridTemplateColumns: "1.22fr 0.78fr", gap: 72, alignItems: "center", marginTop: 56 }}>
